...
main(int argc, char *argv[])
{
    ...
    generalOpts.add_options()("help,h", "Produces this help message")(
        "in,i", po::value<vector<fs::path>>(&seqFiles)->multitoken()->value_name("FILE"),
        "The input file")("index,I", po::value<bool>(&index)->zero_tokens()->default_value(false), "Use an index file")(
    ...
    outputOpts.add_options()("out,o", po::value<fs::path>(&outFile)->value_name("FILE"), "The output file")(
        "append,a", po::value<bool>(&append)->zero_tokens()->default_value(false), "Append sequences to file")(
        "remove-comments,c", po::value<bool>(&removeComments)->zero_tokens()->default_value(false),
        "Remove comments from output")("extract-order",
                                       po::value<bool>(&extractOrder)->zero_tokens()->default_value(false),
    ...
    extractOpts.add_options()("extract,e", po::value<vector<string>>(&extractLine)->multitoken(),
                              "The sequence(s) to extract")("get-names-from-fasta,g",
                                                            po::value<fs::path>(&fastaNamesFile)->value_name("FILE"),
                                                            "Obtain sequences names from fasta")(
        "names-file,N", po::value<fs::path>(&namesFile)->value_name("FILE"), "File with extraction lines to use")(
        "delim-extract,d", po::value<char>(&delimiterExtract)->default_value('\t', "Tab"),
        "The delimiter to use in the extraction file")("column,C", po::value<size_t>(&column)->default_value(1),
                                                       "The column in the extraction file to use")
        //("regex,E", po::value<bool>(&re)->default_value(false)->zero_tokens(), "Interpret extract
        // line as regex")
        ...
        ("remove,r", po::value<bool>(&remove)->default_value(false)->zero_tokens(), "Remove the given sequences")(
            "num-seqs,n", po::value<size_t>(&numSeqs)->default_value(0),
            "The number of randomly choosen sequences to extract")("seed,s", po::value<string>(&seed),
                                                                   "Seed for random extract function")(
            "length,L", po::value<vector<string>>(&lengthStrings)->multitoken(), "Length based extraction")(
            "ignore-missing,m", po::value<bool>(&ignoreMissing)->default_value(false)->zero_tokens(),
            "Ignore missing sequences");

    bool extractSegments;
    po::options_description segOpts("Segment options");
    segOpts.add_options()("extractSegments,S", po::value<bool>(&extractSegments)->zero_tokens()->default_value(false),
                          "Extract segments of a sequence")("delim-pos,D",
                                                            po::value<char>(&delimiterPos)->default_value(':'),
                                                            "The delimiter to use to separate name from positions");
    bool doRevComp, doTranslate;
    string translationTable;
    po::options_description modifyOpts("Modifying options");
    modifyOpts.add_options()("translate,t", po::value<bool>(&doTranslate)->zero_tokens()->default_value(false),
                             "Translate into amino acid")(
        "table,T", po::value<string>(&translationTable)->default_value("standard"), "The translation table to use")(
        "reverse-complement,R", po::value<bool>(&doRevComp)->zero_tokens()->default_value(false),
        "Calculate the reverse complement");

    po::options_description all("seqExtract " + project_version
                                + " (C) 2021  Carsten Kemena\nThis program comes with ABSOLUTELY NO "
    ...
    {
        cerr << "Error! If you provide indexFiles you need to provide them for every sequence file "
                "given\n";
        exit(EXIT_FAILURE);
    }
    ...
    {
        cerr << "An error occurred when parsing the extract arguments! Is there possibly a problem "
                "with the delimiter used for subsection extraction or was no '-' used?\n";
        exit(EXIT_FAILURE);
    }
    ...
    try
    {
                    ...
                    for (auto it = range.first; it != range.second; ++it)
                    {
                        ...
                        if (!it->second.empty())
                        {
                            for (const auto &elem : it->second)
                            {
                                ...
                                name.append((first ? delims : ",") + to_string(start + 1) + "-"
                                            + to_string(elem.second + 1));
                                first = false;
                            }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            // newSet.emplace_back(std::move(set[i].name()), std::move(set[i].seq()), "",
                            // std::move(set[i].comment()));
                            set.clear();
                        }
                    }
                }
        ...
        seqSetIO.write(newSet, "fasta", outFile,
                       mode); //		newSet.write(outFile, removeComments, mode);
    }
    catch (std::exception &e)
    {
