...
main(int argc, char *argv[])
{
    ...
    general.add_options()("help,h", "Produces this help message")(
        ...
        "The sequence file")("out,o", po::value<fs::path>(&outFile)->value_name("FILE"), "The output file")(
        "split-char,s", po::value<char>(&splitChar)->value_name("CHAR")->default_value('-'),
        "The character used for splitting")("summary", po::value<bool>(&summary)->default_value(false)->zero_tokens(),
                                            "Displays a summary of the result");

    fs::path gffFile;
    std::string level1, level2, level3;
    po::options_description gffOpts("GFF options");
    gffOpts.add_options()("gffFile,g", po::value<fs::path>(&gffFile)->value_name("FILE"), "A GFF file")(
        "level1", po::value<std::string>(&level1)->default_value("gene"),
        "Level 1 term")("level2", po::value<std::string>(&level2)->default_value("mRNA"), "Level 2 term")(
    ...
    regexOpts.add_options()("regular,r", po::value<std::string>(&regex), "Regular expression")(
        "name,n", po::value<bool>(&searchName)->default_value(false)->zero_tokens(),
        "Search name only")("comment,c", po::value<bool>(&searchComment)->default_value(false)->zero_tokens(),
                            "Search comment only")("preset,p", po::value<std::string>(&preset), "Preset regex");

    std::string project_version(std::string(PROJECT_VERSION));
    po::options_description visibleOpts("isoformCleaner " + project_version
                                        + " (C) 2019-2022  Carsten Kemena\nThis program comes with ABSOLUTELY NO "
    ...
    if (!preset.empty())
    {
        std::map<std::string, std::string> presetRegex = {
            {"flybase",        "parent=(FBgn[^ ,]+,)"},
            {   "gene", " gene[:=]\\s*([\\S]+)[\\s]*"}
        };
    ...
    if (!gffFile.empty())
    {
        std::function<std::pair<std::string, ISOFORM_STATUS>(BSDL::Sequence)> nameFunc
            = std::bind(nameIdentifier, std::placeholders::_1, names);
        isocleaner.setGeneNameIdentifcator(nameFunc);
    }
    else if (regex.empty())
    {
        std::function<std::pair<std::string, ISOFORM_STATUS>(BSDL::Sequence)> nameFunc
            = std::bind(splitCharIdentifier, std::placeholders::_1, splitChar);
        isocleaner.setGeneNameIdentifcator(nameFunc);
    }
    else
    {
        ...
        std::function<std::pair<std::string, ISOFORM_STATUS>(BSDL::Sequence)> nameFunc
            = std::bind(regexIdentifier, std::placeholders::_1, e, searchComment, searchName);
        isocleaner.setGeneNameIdentifcator(nameFunc);
    }
