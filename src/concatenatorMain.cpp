...
main(int argc, char *argv[])
{
    ...
    general.add_options()("help,h", "Produces this help message")(
        "in,i", po::value<std::vector<fs::path>>(&sequence_files)->multitoken()->value_name("FILE"),
        "The alignment files")("directory,d", po::value<fs::path>(&input_directory)->value_name("DIRECTORY"),
                               "The directory with the alignment files")(
        "ending,e", po::value<std::string>(&ending)->value_name("STRING")->default_value(".fa"),
        "File ending to be used")("lenient,l", po::value<bool>(&lenient)->default_value(false)->zero_tokens(),
                                  "Allows for missing sequences which will be replaced with gaps")(
    ...
    nameDetection.add_options()("delimiter,D", po::value<std::string>(&delimiter)->default_value(""),
                                "Delimiter to use, default take whole sequence name")(
    ...
    po::options_description visibleOpts("concatenator " + project_version
                                        + " (C) 2023  Carsten Kemena\nThis program comes with ABSOLUTELY NO "
