...
{
    throw std::runtime_error("Error! Duplicated sequence name '" + seqName + "' in file '" + fileName.string()
                             + "' found!");
}
...
createMatches(const std::map<std::string, int> &names1, const fs::path &seqFile1,
              const std::map<std::string, int> &names2, const fs::path &seqFile2, bool lenient)
{
    ...
    for (auto &elem : names1)
    {
        ...
        else
        {
            ...
            {
                throw std::runtime_error("Error! Sequence '" + elem.first
                                         + "' not contained in file: " + seqFile2.string());
            }
        }
    }
    for (auto &elem : names2)
    {
        ...
        if (it == names1.end())
        {
            ...
            {
                throw std::runtime_error("Error! Sequence '" + elem.first
                                         + "' not contained in file: " + seqFile1.string());
            }
        }
    }
...
void
concatenate(BioSeqDataLib::SequenceSet &seqSet1, const BioSeqDataLib::SequenceSet &seqSet2,
            const std::set<std::pair<int, int>> &matches)
{
    ...
    for (auto &match : matches)
    {
        if (match.first == -1)
        {
            seqSet1.emplace_back(
                BioSeqDataLib::Sequence(seqSet2[match.second].name(), std::string(currentLength, '-'), "", ""));
