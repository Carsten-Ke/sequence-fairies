...
main(int argc, char *argv[])
{
    ...
    general.add_options()("help,h", "Produces this help message")(
        "in,i", po::value<fs::path>(&sequenceFile)->required()->multitoken()->value_name("FILE"),
        "The sequence file")("out,o", po::value<fs::path>(&outFile)->value_name("FILE"), "The fasta output file")(
        "report,r", po::value<fs::path>(&reportFile)->value_name("FILE"), "The file to store the report in")(
        "check-all,A", po::value<bool>(&checkAll)->default_value(false)->zero_tokens(),
        "run all checks")("fix-and-keep", po::value<bool>(&fixAndKeep)->default_value(false)->zero_tokens(),
                          "Fix all problems without removing a sequence")(
        "fix-and-remove", po::value<bool>(&fixAndRemove)->default_value(false)->zero_tokens(),
        "Fix all Problems by removing sequences (except stop at ends)");
    ...
    stopOpts.add_options()("check-stops", po::value<bool>(&checkStops)->default_value(false)->zero_tokens(),
                           "Checks for stop chars")("fix-ending",
                                                    po::value<bool>(&fixEnd)->default_value(false)->zero_tokens(),
                                                    "Removes all stops at the end of a sequence")(
        "remove-stop-genes", po::value<bool>(&removePseudogenes)->zero_tokens()->default_value(false),
        "Remove all genes with stops")("stop-char", po::value<string>(&stopChars)->default_value(".*"),
                                       "The stop chars to use")(
        "replace-stop", po::value<bool>(&replaceStopChar)->zero_tokens()->default_value(false),
        "Replace in sequence stops with ambiguous char");

    bool checkAlphabet, removeAlpha, replaceAlphaChar;
    std::string alphabet;
    po::options_description alphabetOpts("Alphabet check options");
    alphabetOpts.add_options()("check-alphabet", po::value<bool>(&checkAlphabet)->default_value(false)->zero_tokens(),
                               "Check if a usual alphabet is used")(
        "set-alphabet", po::value<string>(&alphabet)->default_value("protein"), "The alphabet to use")(
        "replace-char", po::value<bool>(&replaceAlphaChar)->default_value(false)->zero_tokens(),
        "Replace weird chars with ambigious one")("remove-alpha",
                                                  po::value<bool>(&removeAlpha)->default_value(false)->zero_tokens(),
    ...
    duplicateOpts.add_options()("check-duplicates",
                                po::value<bool>(&checkDuplicates)->default_value(false)->zero_tokens(),
                                "Check for sequences with the same name")(
        "rename-duplicates", po::value<bool>(&renameDuplicates)->default_value(false)->zero_tokens(),
        "Renames the duplicates")("remove-duplicates",
                                  po::value<bool>(&removeDuplicates)->default_value(false)->zero_tokens(),
                                  "Removes duplicates");

    po::options_description visibleOpts("seqCheck " + project_version
                                        + " (C) 2021-2023  Carsten Kemena\nThis program comes with ABSOLUTELY NO "
    ...
    if (renameDuplicates | replaceAlphaChar | fixEnd | replaceStopChar | removeAlpha | removeDuplicates | fixEnd
        | removePseudogenes)
    {
