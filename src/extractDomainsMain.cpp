...
main(int argc, char *argv[])
{
    ...
    general.add_options()("help,h", "Produces this help message")(
        "domains,d", po::value<std::vector<fs::path>>(&domainFiles)->required()->multitoken()->value_name("FILE"),
        "The domain arrangement file")(
        "in,i", po::value<std::vector<fs::path>>(&sequenceFiles)->required()->multitoken()->value_name("FILE"),
        "The sequence file")("out,o", po::value<fs::path>(&outFile)->value_name("FILE"), "The output file")(
        "name,n", po::value<std::string>(&pattern)->default_value("%s_%b-%e %d"),
        "Pattern to use for the sequence name")("id", po::value<std::string>(&accession)->value_name("ARG"),
                                                "The pfam-id to extract")(
        "min-length", po::value<size_t>(&minLength)->default_value(0), "The minimum length to use")(
        "use-envelope,e", po::value<bool>(&useEnvelope)->default_value(false)->zero_tokens(),
        "Use envelope coordinates if available");

    po::options_description visibleOpts("extractDomains " + project_version
                                        + " (C) 2018-2023  Carsten Kemena\nThis program comes with ABSOLUTELY NO "
