...
@test "split-cleaning" {
    run -0 ../../build/bin/isoformCleaner -i data/isoformCleaner/splitChar.fasta -o splitChar.fa --summary
	...
	run -0 ../../build/bin/isoformCleaner -i data/isoformCleaner/splitChar_fail.fasta -o splitChar_fail.fa
...
@test "regex-cleaning" {
    run ../../build/bin/isoformCleaner -i data/isoformCleaner/regex.fasta -o regex.fa -p gene
	[ $status == 0 ]
    ...
    run ../../build/bin/isoformCleaner -i data/isoformCleaner/regex.fasta -o regex.fa -r "gene[:=]\\s*([\\S]+)[\\s]*"
	[ $status == 0 ]
...
@test "gff-cleaning" {
    run ../../build/bin/isoformCleaner -i data/isoformCleaner/gff3.fasta -g data/isoformCleaner/gff3.gff3 -o gff.fa
	[ $status == 0 ]
	...
	run ! ../../build/bin/isoformCleaner -i data/isoformCleaner/gff3.fasta -g data/isoformCleaner/missing.gff3
...
@test "file-opening-errors" {
    run ../../build/bin/isoformCleaner -i xxx -o splitChar.fa
    [ "$output" == "Error opening file 'xxx': No such file or directory: iostream error" ]
	[ $status != 0 ]

	run ../../build/bin/isoformCleaner -i data/isoformCleaner/regex.fasta -r "gene[:=]\\s*([\\S]+)[\\s]*" -o doesnotexist/x.fa
    [ "$output" = "Error opening file 'doesnotexist/x.fa': No such file or directory: iostream error" ]
}


@test "error messages" {
    run ../../build/bin/isoformCleaner -i data/isoformCleaner/regex.fasta -o regexFail.fa -p flybase
	...
	run -1 ../../build/bin/isoformCleaner -i data/isoformCleaner/regex.fasta -o regexFail.fa -p doesnotexit
	[ "$output" == "Error: Preset 'doesnotexit' is unknown!" ]

	run -1 ../../build/bin/isoformCleaner --doesnotexist
...
@test "help messages" {
    run ../../build/bin/isoformCleaner -h
